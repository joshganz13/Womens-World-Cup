...
pacman::p_load(flexdashboard, tidyverse, ggimage, ggthemes, plotly, cowplot, sf, rworldmap, rgeos)
```

### Amanda Peterson Plunkett #tidytuesday Women's World Cup post. {.no-padding}
...
### Modified Selectioin: interactive visual based on our group's critique. {.no-padding}
```{r message=F, warning=F}
...
### Future visual our group began to incorporate a more geographical theme. We would like to make it interactive with dropdown menus and hovering information about yearly goals, cumulative goals when latter years selected,  country, and year. {.no-padding}
```{r, message=F, warning=F}
